...
- [Go 程序构建基块](#go-程序构建基块)
	...
	- [2. 程序初始化和执行](#2-程序初始化和执行)
		...
		- [2.4. 程序执行](#24-程序执行)
	- [3. 词汇元素](#3-词汇元素)
		- [3.1. Go 关键字和预声明标识符](#31-go-关键字和预声明标识符)
		- [3.2. 运算符与标点符号](#32-运算符与标点符号)
	- [4. 字面值文本](#4-字面值文本)
		- [4.1. 整数文本](#41-整数文本)
		- [4.2. 浮点文本](#42-浮点文本)
		- [4.3. 虚数文本](#43-虚数文本)
	...
	- [6. 常量与变量](#6-常量与变量)
		...
		- [6.2. 变量](#62-变量)
	- [7. 类型](#7-类型)
		- [7.1. Boolean：布尔类型](#71-boolean布尔类型)
		- [7.2. Numeric：数值类型](#72-numeric数值类型)
		- [7.3. String：字符串类型](#73-string字符串类型)
		- [7.4. Array：数组类型](#74-array数组类型)
		- [7.5. Slice：切片类型](#75-slice切片类型)
			- [7.5.1. 切片表达式](#751-切片表达式)
			- [7.5.2. 切片增长与复制](#752-切片增长与复制)
			- [7.5.3. 多维切片](#753-多维切片)
		- [7.6. Struct：结构类型](#76-struct结构类型)
		- [7.7. Function：函数类型](#77-function函数类型)
			- [7.7.1. 函数声明](#771-函数声明)
			- [7.7.2. 可变参数](#772-可变参数)
			- [7.7.3. 函数文本](#773-函数文本)
		- [7.8. Interface：接口类型](#78-interface接口类型)
			- [7.8.1. Base Interfaces](#781-base-interfaces)
			- [7.8.2. Embedded interfaces](#782-embedded-interfaces)
			- [7.8.3. General interfaces](#783-general-interfaces)
			- [7.8.4. 实现接口](#784-实现接口)
		- [7.9. Map：映射类型](#79-map映射类型)
			- [7.9.1. 并发中的映射](#791-并发中的映射)
		- [7.10. Channel：通道类型](#710-channel通道类型)
		- [7.11. Pointer：指针类型](#711-pointer指针类型)
		- [7.12. 命名类型](#712-命名类型)
			- [7.12.1. 别名声明](#7121-别名声明)
			- [7.12.2. 类型定义](#7122-类型定义)
	- [8. 类型和值的属性](#8-类型和值的属性)
		- [8.1. 底层类型](#81-底层类型)
		- [8.2. 核心类型](#82-核心类型)
		- [8.3. 类型等价性](#83-类型等价性)
	...
	- [14. 表达式与运算符](#14-表达式与运算符)
		...
		- [14.4. 算数运算符](#144-算数运算符)
			- [14.4.1. 整数运算符](#1441-整数运算符)
			- [14.4.2. 整数溢出](#1442-整数溢出)
			- [14.4.3. 浮点运算符](#1443-浮点运算符)
			- [14.4.4. 字符串连接](#1444-字符串连接)
		...
		- [14.11. 类型转换](#1411-类型转换)
			- [14.11.1. 数值类型之间的转换](#14111-数值类型之间的转换)
			- [14.11.2. 与字符串之间的转换](#14112-与字符串之间的转换)
			- [14.11.3. 从切片到数组或数组指针的转换](#14113-从切片到数组或数组指针的转换)
...
Go 程序是通过将包链接在一起来构建的。包中的元素（常量、类型、变量和函数）可以导出（大写开头）并在另一个包中使用，"_" 被视为小写。
...
`package` 声明表示当前源文件所属的包，实现可能要求一个包的所有源文件都位于同一个目录中。
...
`import` 声明源文件导入的外部依赖包，由包路径和一个可选的限定访问符组成。
...
在包中，全局变量的初始化按声明顺序进行，该变量与未初始化的变量没有依赖关系。单个表达式的多值赋值中，左值中的多变量在同一步骤中初始化。
...
在多个文件中变量的声明顺序取决于文件提交给编译器的顺序。依赖性分析只依赖于源代码中对它们的词法引用，并进行传递性分析。
...
依赖性分析是针对每个包执行的。如果变量间存在其他隐藏的数据依赖关系，则未指定它们之间的初始化顺序。例如：
...
一个完整程序的包是逐步初始化的，首先初始化外部依赖包，每个包仅初始化一次。在单个 *goroutine* 中依次进行，每次一个包：包初始化 ➡ 变量初始化 ➡ 调用 `init` 函数。函数 `init` 可以启动其他的 *goroutine*，这些 *goroutine* 可以与初始化代码并发运行。初始化总是对 `init` 函数进行排序：在前一个函数返回之前，它不会调用下一个函数。

每个源文件都可以声明任意数目的初始化函数 `init` 来设置所需的任何状态，函数 `init` 在全局变量初始化完成后，按声明顺序调用，最后执行 `main.main()`。名称 `init` 仅用作 `init` 函数标识符，且只能被 Go 运行时调用。
...
一个完整的程序是通过将一个主包（`package main`，非导入包）与它导入的所有包递归地链接起来创建的。当入口函数 `main` 调用返回时，程序退出，它不会等待其他非 *main goroutine* 完成。
...
整数文本是表示整数常量的数字序列。`0b`、`0B` 用于二进制文本前缀；`0` 或 `0o`、`0O` 用于八进制；`0x`、`0X` 用于十六进制。数字分隔符 `_` 可以出现在整数文本前缀之后或连续数字之间。
...
虚数文本表示复数常量的虚数部分。为了向后兼容，仅以前导 0 开头的数字序列被视为十进制数字。
...
字符文本表示一个字符常量，标识一个 Unicode 码位的整数值。Go 源文本使用 UTF-8 编码的 Unicode 字符，因此多个 UTF-8 编码的字节可以表示单个字符。例如字符 `'a'` 包含单个字节（0x61）的表示文本 a U+0061，字符 `'ä'` 包含两个字节（0xc3 0xa4）的表示文本 a-dieresis U+00E4。

有四种方法可以将整数值表示为 Unicode 字符常量。设 *h* 表示十六进制数字，*n* 表示八进制数字：`\x hh`、`\u hhhh`、`\U hhhhhhhh`、`\ nnn`。
...
原始字符串是反引号 `` ` `` 之间未解释的字符序列。反斜杠没有特殊含义，可能包含换行符，回车符被丢弃。
...
解释字符串是双引号 `"` 之间的字符序列。除了 `\'` 可以包含其他转义字符、`\x hh`、`\u hhhh`、`\U hhhhhhhh`、`\ nnn`。
...
声明将非空标识符绑定到常量、类型、类型参数、变量、函数、标签或包。空标识符（`_`）表示弃用，可以分配任意值。

Go 语言使用块来限定词法作用域：
- Go 的作用域层次结构分为 *universe* > *package* > *file* > *function* > *block* 
- 预声明标识符的作用域是 *universe*。
- 文件范围声明的标识符的作用域 *package*，它表示为顶级（任何函数块之外）声明的常量、类型、变量或函数。
- 导入包的范围是其声明的 *file* 范围。
- 表示方法接收器、函数参数或结果变量的标识符的作用域是 *function*。
- 表示函数的类型参数的作用域，开始于函数名之后，结束于函数体的末尾。
- 表示类型的类型参数的标识符的作用域开始于类型名称之后，结束于 `type` 声明的末尾。
- 在函数内部声明的常量或变量标识符的作用域开始于 `const`、`var` 或短变量声明的末尾，结束于其包含块的末尾。
- 在函数内部声明的类型标识符的作用域开始于声明位置，结束于声明块范围的末尾。
- 标签的作用域是声明它的函数体。标签由带标签的语句声明，并在 `break`、`continue` 和 `goto` 语句中使用。

局部变量的标识符可以在内部块中重新声明。`package` 声明的包名称不出现在任何范围中，它用于标记属于同一个包的文件，并作为 `import` 导入时使用的默认包名。

>---

#### 5.1. 空标识符

空标识符 `_` 用于匿名占位符，表示该值将被无害地丢弃。例如在多值赋值时，放弃接收其中的某个值，以避免创建一个伪变量。
...
包的匿名导入表示只是为了它的副作用，而不使用它的任何导出标识符。仅因其副作用而导入的包应仅在程序的主包中导入，或在需要它们的测试中导入。
...
常量有布尔常量、数值常量（字符常量、整数常量、浮点常量、复数常量）和字符串常量。数值常量表示任意精度的精确值，并且不会溢出。没有常量表示 IEEE-754 负零、无穷大和非数字值。`iota` 表示为整数常量。

常量可以是类型化（显式指定）的，也可以是非类型（隐式）的。文本常量、`true`、`false`、`iota` 和某些只包含非类型化常量操作数的常量表达式是非类型化的。非类型化常量的默认类型分别是 `bool`、`rune`、`int`、`float64`、`complex128` 或 `string`。
...
Go 是静态类型语言，不允许混合数值类型的操作，数值类型之间不存在隐式转换。例如不能将 `float64` 和 `int` 类型的值进行混合算数运算，不同类型之间必须强制类型转换（`int(f64) + i` 或 `f64 + float64(i)`）。非类型化常量值可以在其他表达式中隐式转换为目标类型。
...
Go 语言中非类型化常量的概念意味着所有的数值常量，无论是整数、浮点数、复数还是字符值，都存在于一种统一的空间中。尽管它们具有不同的隐式默认类型，但作为非类型化常量，它们可以被分配给任何数值类型的变量：
...
在 `const` 声明中，标识符 `iota` 表示从零开始的连续的无类型整数常量。未指定初始值设定项的标识符，将复用上一个包含 `iota` 的常量表达式作为初始化表达式。在 Go 中，枚举常量是使用 `iota` 枚举器创建的。它可以用来构造一组相关的常量：
...
变量是用于保存值的存储位置，允许的值集由变量类型决定。调用内置函数 `new` 或获取复合文本的地址会在运行时为变量分配堆存储。数组、切片和结构类型的变量具有可以单独寻址的元素和字段。

变量的静态类型是其声明类型或表达式赋值的结果类型。接口类型的变量可以具有不同的动态类型，即在运行时分配给变量的值的（非接口）类型。
...
变量声明创建一到多个变量，函数体内未使用的变量声明可能是非法的。`nil` 不能用于初始化没有显式类型声明的变量。
...
函数作用域可以使用短变量声明：`idenList := exprList`。可以重新声明短变量，但要求左侧至少有一个新声明的非空变量。
...
变量的存储位置对编写高效程序有影响。如果可能的话，Go 编译器将在函数运行的 goroutine 堆栈中分配函数的局部变量。如果编译器无法证明函数返回后变量没有被引用，则编译器必须在垃圾收集堆上分配变量以避免悬空指针错误。

在当前编译器中，如果一个变量的地址被获取，那么这个变量就是堆上分配的候选变量。如果一个基本的逃逸分析可以识别出某些情况，例如这些变量在函数返回后确定不会存在，则可以在堆栈上驻留。
...
布尔类型（`bool`）表示由预先声明的常量 `true` 和 `false` 表示的布尔值集。
...
使用不同的数值类型时，需要显式转换。`int32` 和 `int` 在特定体系结构上可能具有相同的大小，即使它们不是同一类型。
...
字符串类型（`string`）表示字符串值的集合。字符串值是一个（可能为空的）字节序列。内置函数 `len` 获取字符串的长度，字符串不可变。字符串的字节（`byte`）可以通过整数 `0` 到 `len(s)-1` 索引，字符串字节元素不可寻址。字符串字节不代表单个字符，遍历字符应使用字符类型（`rune`）。
...
数组是单一类型元素的编号序列。内置函数 `len` 来获取数组的元素数目。元素可以通过整数 `0` 到 `len(a)-1` 索引，数组元素可寻址。Go 的数组具有值属性，传递数组时将传递源数组的副本。
...
数组类型 `T` 不能具有 `type T` 的元素，也不能直接或间接包含 `T` 为组件的类型。
...
切片是其底层数组某个连续段的描述符，提供对该数组段中元素的编号序列的访问，它由一个指向数组的指针、段的长度及其容量（段的最大长度）组成。
未初始化切片的值为 `nil`。
...
切片是通过指定一个半开放范围来完成的。表达式 `a[1:4]` 创建一个包含 `a` 的元素 1 到 3 的切片（结果切片的索引将是 0 到 2）。
...
切片包含对底层数组的引用，函数对于切片元素所做的修改对调用方来说是可见的。在函数中，切片按值传递。
...
切片 `s` 的长度可以通过函数 `len` 获取；切片的长度可能会在执行过程中更改。切片一旦初始化，将始终与保存其元素的底层数组相关联。内置函数 `cap(s)` 来获取切片 `s` 的容量。值为 `nil` 的切片，其 `len` 和 `cap` 均为 0。
...
使用内置函数 `make` 为给定元素类型 `T` 创建一个新的、长度为 `length` 和可选容量大小（`capacity` 为底层数组长度，默认为 `length`）、零值初始化的切片值。创建切片的 `make` 函数始终为切片绑定一个新的隐藏数组。
...
切片表达式从字符串、数组、指向数组的指针、或切片中构造子字符串或切片。
...
`a[low : high]` 的核心类型是字符串、数组、指向数组的指针、切片、或 `bytestring`。满足关系式：`0 <= low <= high <= cap(a)`。可以省略任何索引：缺失的 `low` 索引默认为零；缺失的 `high` 索引默认为切片操作数的长度。

`a[low : high : max]` 的核心类型是数组、指向数组的指针、切片。只有第一个索引可以省略。满足关系式：`0 <= low <= high <= max <= cap(a)`。`a[low : high : max]` 构造一个与简单切片表达式 `a[low : high]` 相同类型、相同长度和元素的切片，`max - low` 来控制结果切片的容量。

如果 `a` 是指向数组的指针，则 `a[low : high]` 是 `(*a)[low : high]` 的简写；`a[low : high : max]` 是 `(*a)[low : high : max]` 的简写。
...
使用内置函数 `copy` 将数据从源切片复制到目标切片，并返回复制的元素数。`copy` 函数支持在不同长度的切片之间进行复制（它只复制最多较短切片长度数目的元素）。`copy` 可以处理共享相同底层数组的源和目标切片，并正确处理重叠切片。
...
切片始终是一维的，但可以组合成构造更高维的对象。对于数组的数组，内部数组在构造上始终是相同的长度；但对于切片的切片（或切片阵列），内部长度可能会动态变化。内部切片必须单独初始化。
...
结构包含一系列的命名元素字段。字段名称可以显式指定（*IdentifierList*）或隐式指定（*EmbeddedField*）。非空字段的名称必须是唯一的。可以使用空标识符 `_` 在字段之间填充字节。
...
使用类型声明但没有显式字段名称的字段称为嵌入字段（*EmbeddedField*）。嵌入字段必须指定为类型名称 `T` 或指向非接口类型名称 `*T` 的指针，并且 `T` 其本身可能不是指针类型。非限定类型名称充当字段名称。
...
如果 `x.f` 是表示该字段 `f` 或方法 `f` 的合法选择器（*selector*），则结构体 `x` 中嵌入字段的字段 `f` 或方法 `f` 被称为提升（*promoted*）。提升字段与普通字段类似，但不能用作结构的复合文本中的字段名称。相同的名称不能出现在相同的嵌套级别，更浅层次的名称成员将覆盖更深层次的相同名称成员。
...
函数类型表示具有相同参数和结果类型的所有函数的集合。未初始化的函数值为 `nil`。函数可以返回多个值。
...
在参数或结果列表中，标识符名称必须全部存在或全部不存在。可变参数 `...` 位于参数列表的最后一个。
...
与 C 家族的所有语言一样，Go 中的所有内容都是通过值传递的。也就是说，一个函数总是得到一个正在传递的内容的副本。
...
如果函数声明指定了类型参数，则函数表示为泛型函数。泛型函数使用前必须先实例化 。
...
如果 `f` 函数包含可变参数，即它的最后一个参数 `p` 的类型为 `...T`，则在 `f` 中，`p` 的类型等价于类型 `[]T`。如果调用 `f` 时没有为 `p` 提供实际参数，则传递给 `p` 的值为 `nil`。
...
在 GO 1.22 之前，由于每次迭代在闭包中都使用一个变量 v 的同一实例，因此最终的输出可能是 `c, c, c`。也就是说自 goroutine 启动时，闭包变量已经发生了改变。这种语言的行为，没有为每次迭代定义一个新变量，在回顾时被认为是一个错误，并且在 Go 1.22 中得到了解决，它确实为每次迭代创建了一个新变量，并消除了这个问题。
