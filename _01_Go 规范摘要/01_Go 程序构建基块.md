...
每个源文件开头都包含一个 `package` 声明并置于文件开头。实现可能要求一个包的所有源文件都位于同一个目录中。

```go
package myPack
```

`import` 导入依赖的外部包集，包含外部包的导入路径和一个可选的限定访问标识符。。
...
在包中，包级变量的初始化按声明顺序进行，该变量与未初始化的变量没有依赖关系。右侧单个表达式的多值赋值中，左侧的多个变量在同一步骤中初始化。
...
每个源文件都可以声明任意数目的初始化函数 `init` 来设置所需的任何状态。函数 `init` 在包中的所有变量初始化完成后调用。保留标识符 `init` 只用于声明 `init` 函数，且无法被调用。

整个包的初始化首先初始化所有全局变量，然后按声明顺序调用所有 `init` 函数。最后执行 `main` 包的入口函数 `main()`。
...
一个完整程序的包是逐步初始化的，首先初始化外部依赖包，不会重复初始化。在单个 *goroutine* 中依次进行，每次一个包：包初始化 ➡ 变量初始化 ➡ 调用 `init` 函数。

函数 `init` 可以启动其他的 *goroutine*，这些 *goroutine* 可以与初始化代码并发运行。初始化总是对 `init` 函数进行排序：在前一个函数返回之前，它不会调用下一个函数。
...
字符文本表示一个字符常量，标识一个 Unicode 码位的整数值。Go 源文本使用 UTF-8 编码的 Unicode 字符，因此多个 UTF-8 编码的字节可以表示单个整数值。

字符 `'a'` 包含单个字节（0x61）的表示文本 a Unicode U+0061，字符 `'ä'` 包含两个字节（0xc3 0xa4）的表示文本 a-dieresis，U+00E4，值 0xe4。
...
原始字符串是反引号 `` ` `` 之间未解释的字符序列。反斜杠没有特殊含义，字符串可能包含换行符。文本中的回车符 （`'\r'`） 将从原始字符串值中丢弃。
...
解释字符串是双引号 `"` 之间的字符序列。反斜杠转义被解释为字符文本的文本（除了 `\'`）。`\ nnn` 和 `\x hh` 转义表示为结果字符串的单个字节；所有其他转义都表示单个字符的（可能是多字节）UTF-8 编码。
...
声明将非空标识符绑定到常量、类型、类型参数、变量、函数、标签或包。空标识符（`_`）表示弃用，可以分配任意值。标识符 `init` 只能用于 `init` 函数声明。
...
局部标识符可以在内部块中重新声明。包名称不出现在任何范围中，它用于标记属于同一个包的文件，并为 `import` 声明指定默认的包名。
...
常量有布尔常量、数值常量（字符常量、整数常量、浮点常量、复数常量）和字符串常量。数值常量表示任意精度的精确值，并且不会溢出。没有常量表示 IEEE-754 负零、无穷大和非数字值。预先声明的 `iota` 表示整数常量。

常量可以是类型化（显式指定）的，也可以是非类型（隐式）的。文本常量、`true`、`false`、`iota` 和某些只包含非类型化常量操作数的常量表达式是非类型化的。

非类型化常量的默认类型分别是 `bool`、`rune`、`int`、`float64`、`complex128` 或 `string`。
...
Go 是一种静态类型语言，不允许混合数字类型的操作。例如不能将 `float64` 和 `int` 类型的值进行混合算数运算，不同类型之间必须强制类型转换，即 `int(f64) + i` 或 `f64 + float64(i)`。非类型化的常量文本值在合法使用的地方隐式转换为目标类型。
...
变量是用于保存值的存储位置，允许的值集由变量类型决定。调用内置函数 `new` 或获取复合文本的地址会在运行时为变量分配存储。数组、切片和结构类型的变量具有可以单独寻址的元素和字段。

变量的静态类型是其声明类型或初始化表达式的结果类型。接口类型的变量也具有不同的动态类型，即在运行时分配给变量的值的（非接口）类型。
...
函数内部的短变量声明使用 `idenList := exprList` 形式。短变量重新声明时，要求左侧至少有一个非空变量是新声明的变量。
...
变量的存储位置对编写高效程序有影响。如果可能的话，Go 编译器将在函数的堆栈框架中分配函数的局部变量。但是如果编译器无法证明函数返回后变量没有被引用，则编译器必须在垃圾收集堆上分配变量以避免悬空指针错误。此外如果局部变量非常大，将其存储在堆中而不是堆栈中可能更有意义。

在当前的编译器中，如果一个变量的地址被获取，那么这个变量就是堆上分配的候选变量。然而，一个基本的转义分析可以识别出某些情况，即这些变量在函数返回后不会存在，则可以驻留在堆栈上。
...
布尔类型表示由预先声明的常量 `true` 和 `false` 表示的布尔真值集。预先声明的布尔类型是 `bool`。
...
字符串类型表示字符串值的集合。字符串值是一个（可能为空的）字节序列。内置函数 `len` 获取字符串的长度，字符串不可变。预先声明的字符串类型为 `string`，它是一个定义类型。

字符串的字节可以通过整数 `0` 到 `len(s)-1` 索引，单字节用 `byte` 类型表示。字符串单字节元素不可寻址。
...
数组是单一类型元素的编号序列。内置函数 `len` 来获取数组的长度。元素可以通过整数索引 `0` 到 `len(a)-1` 来寻址。Go 的数组具有值属性，传递数组时将传递源数组的副本。
...
切片是其底层数组某个连续段的描述符，提供对该数组段中元素的编号序列的访问。未初始化切片的值为 `nil`。
...
切片包含对底层数组的引用。如果一个函数接受一个切片参数，那么它对切片元素所做的修改对调用方来说是可见的。在函数中，切片按值传递。
...
切片 `s` 的长度可以通过函数 `len` 获取；切片的长度可能会在执行过程中更改。切片一旦初始化，将始终与保存其元素的底层数组相关联。

内置函数 `cap(s)` 来获取切片 `s` 的容量。值为 `nil` 的切片，其 `len` 和 `cap` 均为 0。
...
使用内置函数 `copy` 将数据从源切片复制到目标切片，并返回复制的元素数。`copy` 函数支持在不同长度的切片之间进行复制（它只复制最多较短切片长度数目的元素）。

`copy` 可以处理共享相同底层数组的源和目标切片，并正确处理重叠切片。
...
切片始终是一维的，但可以组合成构造更高维的对象。对于数组的数组，内部数组在构造上始终是相同的长度；但是对于切片的切片（或切片阵列），内部长度可能会动态变化。内部切片必须单独初始化。
...
联合（具有超过一个项）不能包含预先声明的标识符 `comparable` 或嵌入 `comparable` 的指定方法（或包含该特定方法的接口）。
...
Go 语言提供了一个内置的 `map` 映射类型来实现哈希表。映射是一种由键值对组成的无序组。映射类型是引用类型，未初始化的映射的值为 `nil`。
...
映射包含对底层数据结构的引用。如果将一个映射传递给一个改变映射内容的函数，这些改变将在调用方中可见。
...
`nil` 映射在读取时的行为类似于空映射，不能添加任何元素，尝试写入 `nil` 映射将导致运行时异常。要初始化使用内置函数 `make`，它创建一个新的空映射值，该函数将映射类型和可选的容量提示作为参数。初始容量不受其大小限制：映射会随着存储项的数量而增长，但 `nil` 映射除外。
...
获取不存在的键的值，映射将返回映射值类型的零值。多重赋值可以检查映射是否包含该键。若 `timeZone` 不包含 `tz`，将设置 `rt` 为其类型零值，且 `ok` 设为 `false`；若读取成功则为 `true`，并返回相应的值给 `rt`。
...
类型参数列表声明泛型函数或类型声明的类型参数。当泛型类型的类型参数列表声明了一个带有约束 `C` 的单个类型参数 `P`，使得文本 `P C` 形成一个有效表达式时，就会出现解析二义性：
