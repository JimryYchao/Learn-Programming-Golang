...
切片包含对底层数组的引用。如果一个函数接受一个切片参数，那么它对切片元素所做的修改对调用方来说是可见的。
...
如果 `x.f` 是表示该字段 `f` 或方法 `f` 的合法选择器（*selector*），则结构体 `x` 中嵌入字段的字段 `f` 或方法 `f` 被称为提升（*promoted*）。提升字段与普通字段类似，但不能用作结构的复合文本中的字段名称。
